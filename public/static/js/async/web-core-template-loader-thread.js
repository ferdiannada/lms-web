(() => {
  'use strict';
  var e,
    t,
    n,
    r,
    _,
    i,
    o,
    s,
    a,
    l,
    c = {
      9099(e, t, n) {
        e.exports = n.p + 'static/wasm/bae681cbe3.module.wasm';
      },
      8331(e, t, n) {
        e.exports = n.p + 'static/wasm/e4df7230ae.module.wasm';
      },
      8235(e, t, n) {
        let r;
        n.r(t);
        class _ {
          static __wrap(e) {
            let t = Object.create(_.prototype);
            return ((t.__wbg_ptr = e), m.register(t, t.__wbg_ptr, t), t);
          }
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), m.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_eventinfo_free(e, 0);
          }
          get event_handler() {
            return r.__wbg_get_eventinfo_event_handler(this.__wbg_ptr);
          }
          get event_name() {
            let e, t;
            try {
              let n = r.__wbg_get_eventinfo_event_name(this.__wbg_ptr);
              return ((e = n[0]), (t = n[1]), L(n[0], n[1]));
            } finally {
              r.__wbindgen_free(e, t, 1);
            }
          }
          get event_type() {
            let e, t;
            try {
              let n = r.__wbg_get_eventinfo_event_type(this.__wbg_ptr);
              return ((e = n[0]), (t = n[1]), L(n[0], n[1]));
            } finally {
              r.__wbindgen_free(e, t, 1);
            }
          }
          set event_handler(e) {
            r.__wbg_set_eventinfo_event_handler(this.__wbg_ptr, e);
          }
          set event_name(e) {
            let t = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              n = H;
            r.__wbg_set_eventinfo_event_name(this.__wbg_ptr, t, n);
          }
          set event_type(e) {
            let t = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              n = H;
            r.__wbg_set_eventinfo_event_type(this.__wbg_ptr, t, n);
          }
        }
        Symbol.dispose && (_.prototype[Symbol.dispose] = _.prototype.free);
        class i {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), v.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_mainthreadwasmcontext_free(e, 0);
          }
          add_closure_event(e, t, n, _, i) {
            let o = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              s = H,
              a = z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              l = H;
            r.mainthreadwasmcontext_add_closure_event(
              this.__wbg_ptr,
              e,
              o,
              s,
              a,
              l,
              M(_) ? 0 : C(_),
              M(i) ? 0 : C(i),
            );
          }
          add_cross_thread_event(e, t, n, _) {
            let i = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              o = H,
              s = z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              a = H;
            var l = M(_) ? 0 : z(_, r.__wbindgen_malloc, r.__wbindgen_realloc),
              c = H;
            r.mainthreadwasmcontext_add_cross_thread_event(
              this.__wbg_ptr,
              e,
              i,
              o,
              s,
              a,
              l,
              c,
            );
          }
          add_dataset(e, t, n) {
            let _ = r.mainthreadwasmcontext_add_dataset(
              this.__wbg_ptr,
              e,
              t,
              n,
            );
            if (_[1]) throw $(_[0]);
          }
          add_run_worklet_event(e, t, n, _) {
            let i = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              o = H,
              s = z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              a = H;
            r.mainthreadwasmcontext_add_run_worklet_event(
              this.__wbg_ptr,
              e,
              i,
              o,
              s,
              a,
              M(_) ? 0 : C(_),
            );
          }
          common_event_handler(e, t, n, _) {
            let i = P(t, r.__wbindgen_malloc),
              o = H,
              s = z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              a = H;
            r.mainthreadwasmcontext_common_event_handler(
              this.__wbg_ptr,
              e,
              i,
              o,
              s,
              a,
              _,
            );
          }
          create_element_common(e, t, n, _, i) {
            var o = M(i) ? 0 : z(i, r.__wbindgen_malloc, r.__wbindgen_realloc),
              s = H;
            return (
              r.mainthreadwasmcontext_create_element_common(
                this.__wbg_ptr,
                e,
                t,
                n,
                M(_) ? Number.MAX_SAFE_INTEGER : 0 | _,
                o,
                s,
              ) >>> 0
            );
          }
          dispatch_event_by_path(e, t, n, _) {
            let i = P(e, r.__wbindgen_malloc),
              o = H,
              s = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              a = H;
            return (
              0 !==
              r.mainthreadwasmcontext_dispatch_event_by_path(
                this.__wbg_ptr,
                i,
                o,
                s,
                a,
                n,
                _,
              )
            );
          }
          dispatch_global_bind_event(e, t, n) {
            let _ = P(e, r.__wbindgen_malloc),
              i = H,
              o = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              s = H;
            r.mainthreadwasmcontext_dispatch_global_bind_event(
              this.__wbg_ptr,
              _,
              i,
              o,
              s,
              n,
            );
          }
          gc() {
            r.mainthreadwasmcontext_gc(this.__wbg_ptr);
          }
          get_component_id(e) {
            let t,
              n = r.mainthreadwasmcontext_get_component_id(this.__wbg_ptr, e);
            if (n[3]) throw $(n[2]);
            return (
              0 !== n[0] &&
                ((t = L(n[0], n[1]).slice()),
                r.__wbindgen_free(n[0], +n[1], 1)),
              t
            );
          }
          get_config(e) {
            let t = r.mainthreadwasmcontext_get_config(this.__wbg_ptr, e);
            if (t[2]) throw $(t[1]);
            return $(t[0]);
          }
          get_data_by_key(e, t) {
            let n = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H,
              i = r.mainthreadwasmcontext_get_data_by_key(
                this.__wbg_ptr,
                e,
                n,
                _,
              );
            if (i[2]) throw $(i[1]);
            return $(i[0]);
          }
          get_dataset(e) {
            let t = r.mainthreadwasmcontext_get_dataset(this.__wbg_ptr, e);
            if (t[2]) throw $(t[1]);
            return $(t[0]);
          }
          get_dom_by_unique_id(e) {
            return r.mainthreadwasmcontext_get_dom_by_unique_id(
              this.__wbg_ptr,
              e,
            );
          }
          get_element_config(e) {
            let t = r.mainthreadwasmcontext_get_element_config(
              this.__wbg_ptr,
              e,
            );
            if (t[2]) throw $(t[1]);
            return $(t[0]);
          }
          get_event(e, t, n) {
            let _ = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              i = H,
              o = z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              s = H;
            return r.mainthreadwasmcontext_get_event(
              this.__wbg_ptr,
              e,
              _,
              i,
              o,
              s,
            );
          }
          get_events(e) {
            let t = r.mainthreadwasmcontext_get_events(this.__wbg_ptr, e);
            var n = A(t[0], t[1]).slice();
            return (r.__wbindgen_free(t[0], 4 * t[1], 4), n);
          }
          get_unique_id_by_component_id(e) {
            let t = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              n = H,
              _ = r.mainthreadwasmcontext_get_unique_id_by_component_id(
                this.__wbg_ptr,
                t,
                n,
              );
            return _ === Number.MAX_SAFE_INTEGER ? void 0 : _;
          }
          constructor(e, t, n) {
            const _ = r.mainthreadwasmcontext_new(e, t, n);
            return (
              (this.__wbg_ptr = _),
              v.register(this, this.__wbg_ptr, this),
              this
            );
          }
          push_style_sheet(e, t) {
            U(e, c);
            var n = M(t) ? 0 : z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H;
            let i = r.mainthreadwasmcontext_push_style_sheet(
              this.__wbg_ptr,
              e.__wbg_ptr,
              n,
              _,
            );
            if (i[1]) throw $(i[0]);
          }
          set_config(e, t) {
            let n = r.mainthreadwasmcontext_set_config(this.__wbg_ptr, e, t);
            if (n[1]) throw $(n[0]);
          }
          set_css_id(e, t, n) {
            let _ = P(e, r.__wbindgen_malloc),
              i = H;
            var o = M(n) ? 0 : z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
              s = H;
            let a = r.mainthreadwasmcontext_set_css_id(
              this.__wbg_ptr,
              _,
              i,
              t,
              o,
              s,
            );
            if (a[1]) throw $(a[0]);
          }
          set_dataset(e, t, n) {
            let _ = r.mainthreadwasmcontext_set_dataset(
              this.__wbg_ptr,
              e,
              t,
              n,
            );
            if (_[1]) throw $(_[0]);
          }
          set_page_element_unique_id(e) {
            r.mainthreadwasmcontext_set_page_element_unique_id(
              this.__wbg_ptr,
              e,
            );
          }
          take_timing_flags() {
            let e = r.mainthreadwasmcontext_take_timing_flags(this.__wbg_ptr);
            var t = A(e[0], e[1]).slice();
            return (r.__wbindgen_free(e[0], 4 * e[1], 4), t);
          }
          update_component_css_id(e, t) {
            let n = r.mainthreadwasmcontext_update_component_css_id(
              this.__wbg_ptr,
              e,
              t,
            );
            if (n[1]) throw $(n[0]);
          }
          update_component_id(e, t) {
            var n = M(t) ? 0 : z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H;
            let i = r.mainthreadwasmcontext_update_component_id(
              this.__wbg_ptr,
              e,
              n,
              _,
            );
            if (i[1]) throw $(i[0]);
          }
          update_css_og_style(e, t) {
            var n = M(t) ? 0 : z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H;
            let i = r.mainthreadwasmcontext_update_css_og_style(
              this.__wbg_ptr,
              e,
              n,
              _,
            );
            if (i[1]) throw $(i[0]);
          }
        }
        Symbol.dispose && (i.prototype[Symbol.dispose] = i.prototype.free);
        class o {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), x.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_rawstyleinfo_free(e, 0);
          }
          append_import(e, t) {
            r.rawstyleinfo_append_import(this.__wbg_ptr, e, t);
          }
          constructor() {
            const e = r.rawstyleinfo_new();
            return (
              (this.__wbg_ptr = e),
              x.register(this, this.__wbg_ptr, this),
              this
            );
          }
          push_rule(e, t) {
            U(t, s);
            var n = t.__destroy_into_raw();
            r.rawstyleinfo_push_rule(this.__wbg_ptr, e, n);
          }
        }
        Symbol.dispose && (o.prototype[Symbol.dispose] = o.prototype.free);
        class s {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), S.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_rule_free(e, 0);
          }
          constructor(e) {
            const t = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              n = H,
              _ = r.rule_new(t, n);
            if (_[2]) throw $(_[1]);
            return (
              (this.__wbg_ptr = _[0]),
              S.register(this, this.__wbg_ptr, this),
              this
            );
          }
          push_declaration(e, t) {
            let n = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H,
              i = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              o = H;
            r.rule_push_declaration(this.__wbg_ptr, n, _, i, o);
          }
          push_rule_children(e) {
            U(e, s);
            var t = e.__destroy_into_raw();
            r.rule_push_rule_children(this.__wbg_ptr, t);
          }
          set_prelude(e) {
            U(e, a);
            var t = e.__destroy_into_raw();
            r.rule_set_prelude(this.__wbg_ptr, t);
          }
        }
        Symbol.dispose && (s.prototype[Symbol.dispose] = s.prototype.free);
        class a {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), E.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_ruleprelude_free(e, 0);
          }
          constructor() {
            const e = r.ruleprelude_new();
            return (
              (this.__wbg_ptr = e),
              E.register(this, this.__wbg_ptr, this),
              this
            );
          }
          push_selector(e) {
            U(e, l);
            var t = e.__destroy_into_raw();
            r.ruleprelude_push_selector(this.__wbg_ptr, t);
          }
        }
        Symbol.dispose && (a.prototype[Symbol.dispose] = a.prototype.free);
        class l {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), O.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_selector_free(e, 0);
          }
          constructor() {
            const e = r.selector_new();
            return (
              (this.__wbg_ptr = e),
              O.register(this, this.__wbg_ptr, this),
              this
            );
          }
          push_one_selector_section(e, t) {
            let n = z(e, r.__wbindgen_malloc, r.__wbindgen_realloc),
              _ = H,
              i = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
              o = H,
              s = r.selector_push_one_selector_section(
                this.__wbg_ptr,
                n,
                _,
                i,
                o,
              );
            if (s[1]) throw $(s[0]);
          }
        }
        Symbol.dispose && (l.prototype[Symbol.dispose] = l.prototype.free);
        class c {
          __destroy_into_raw() {
            let e = this.__wbg_ptr;
            return ((this.__wbg_ptr = 0), R.unregister(this), e);
          }
          free() {
            let e = this.__destroy_into_raw();
            r.__wbg_stylesheetresource_free(e, 0);
          }
          constructor(e, t) {
            const n = r.stylesheetresource_new(e, t);
            if (n[2]) throw $(n[1]);
            return (
              (this.__wbg_ptr = n[0]),
              R.register(this, this.__wbg_ptr, this),
              this
            );
          }
        }
        function u(e, t, n) {
          let _ = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
            i = H;
          var o = M(n) ? 0 : z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
            s = H;
          r.add_inline_style_raw_string_key(e, _, i, o, s);
        }
        function f(e, t, n, _, i, o) {
          var s = M(t) ? 0 : z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
            a = H;
          let l = r.decode_style_info(e, s, a, n, _, i, o);
          if (l[2]) throw $(l[1]);
          return $(l[0]);
        }
        function b(e, t, n, _, i, s) {
          U(e, o);
          var a = e.__destroy_into_raw(),
            l = M(n) ? 0 : z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
            c = H;
          let u = r.encode_legacy_json_generated_raw_style_info(
            a,
            t,
            l,
            c,
            _,
            i,
            s,
          );
          if (u[2]) throw $(u[1]);
          return $(u[0]);
        }
        function d(e) {
          let t, n;
          try {
            let o = r.get_font_face_content(e);
            var _ = o[0],
              i = o[1];
            if (o[3]) throw ((_ = 0), (i = 0), $(o[2]));
            return ((t = _), (n = i), L(_, i));
          } finally {
            r.__wbindgen_free(t, n, 1);
          }
        }
        function g(e) {
          let t, n;
          try {
            let o = r.get_style_content(e);
            var _ = o[0],
              i = o[1];
            if (o[3]) throw ((_ = 0), (i = 0), $(o[2]));
            return ((t = _), (n = i), L(_, i));
          } finally {
            r.__wbindgen_free(t, n, 1);
          }
        }
        function w(e, t, n, _, i) {
          let o = D(t, r.__wbindgen_malloc),
            s = H;
          r.set_inline_styles_in_key_value_vec(e, o, s, n, _, i);
        }
        function p(e, t, n, _, i) {
          let o = z(t, r.__wbindgen_malloc, r.__wbindgen_realloc),
            s = H;
          return 0 !== r.set_inline_styles_in_str(e, o, s, n, _, i);
        }
        function h(e, t, n) {
          var _ = M(n) ? 0 : z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
            i = H;
          r.set_inline_styles_number_key(e, t, _, i);
        }
        function y() {
          return {
            __proto__: null,
            './client_bg.js': {
              __proto__: null,
              __wbg_Error_92b29b0548f8b746: function (e, t) {
                return Error(L(e, t));
              },
              __wbg___wbindgen_debug_string_c25d447a39f5578f: function (e, t) {
                let n = z(
                    (function e(t) {
                      let n,
                        r = typeof t;
                      if ('number' == r || 'boolean' == r || null == t)
                        return `${t}`;
                      if ('string' == r) return `"${t}"`;
                      if ('symbol' == r) {
                        let e = t.description;
                        return null == e ? 'Symbol' : `Symbol(${e})`;
                      }
                      if ('function' == r) {
                        let e = t.name;
                        return 'string' == typeof e && e.length > 0
                          ? `Function(${e})`
                          : 'Function';
                      }
                      if (Array.isArray(t)) {
                        let n = t.length,
                          r = '[';
                        n > 0 && (r += e(t[0]));
                        for (let _ = 1; _ < n; _++) r += ', ' + e(t[_]);
                        return r + ']';
                      }
                      let _ = /\[object ([^\]]+)\]/.exec(toString.call(t));
                      if (!_ || !(_.length > 1)) return toString.call(t);
                      if ('Object' == (n = _[1]))
                        try {
                          return 'Object(' + JSON.stringify(t) + ')';
                        } catch (e) {
                          return 'Object';
                        }
                      return t instanceof Error
                        ? `${t.name}: ${t.message}
${t.stack}`
                        : n;
                    })(t),
                    r.__wbindgen_malloc,
                    r.__wbindgen_realloc,
                  ),
                  _ = H;
                (k().setInt32(e + 4, _, !0), k().setInt32(e + 0, n, !0));
              },
              __wbg___wbindgen_is_null_ea9085d691f535d3: function (e) {
                return null === e;
              },
              __wbg___wbindgen_is_undefined_c05833b95a3cf397: function (e) {
                return void 0 === e;
              },
              __wbg___wbindgen_jsval_eq_e659fcf7b0e32763: function (e, t) {
                return e === t;
              },
              __wbg___wbindgen_string_get_b0ca35b86a603356: function (e, t) {
                let n = 'string' == typeof t ? t : void 0;
                var _ = M(n)
                    ? 0
                    : z(n, r.__wbindgen_malloc, r.__wbindgen_realloc),
                  i = H;
                (k().setInt32(e + 4, i, !0), k().setInt32(e + 0, _, !0));
              },
              __wbg___wbindgen_throw_344f42d3211c4765: function (e, t) {
                throw Error(L(e, t));
              },
              __wbg_addEventListener_514c0d1b9cb98d61: function (e, t, n) {
                e.addEventListener(L(t, n));
              },
              __wbg_appendChild_f553e8704c4f14a6: function () {
                return F(function (e, t) {
                  return e.appendChild(t);
                }, arguments);
              },
              __wbg_cloneNode_5f99da4333e10617: function () {
                return F(function (e, t) {
                  return e.cloneNode(0 !== t);
                }, arguments);
              },
              __wbg_createElement_fcbc0805de826d62: function () {
                return F(function (e, t, n) {
                  return e.createElement(L(t, n));
                }, arguments);
              },
              __wbg_cssRules_a96ba5e195723d36: function () {
                return F(function (e) {
                  return e.cssRules;
                }, arguments);
              },
              __wbg_deref_e6425a8fa9d03a9d: function (e) {
                let t = e.deref();
                return M(t) ? 0 : C(t);
              },
              __wbg_disableElementEvent_00750da2a3d98d3b: function () {
                return F(function (e, t, n, r) {
                  e.disableElementEvent(t, L(n, r));
                }, arguments);
              },
              __wbg_enableElementEvent_d3b114945b78a398: function () {
                return F(function (e, t, n, r) {
                  e.enableElementEvent(t, L(n, r));
                }, arguments);
              },
              __wbg_eventinfo_new: function (e) {
                return _.__wrap(e);
              },
              __wbg_getClassList_4ef38849a4003220: function () {
                return F(function (e, t, n) {
                  let _ = D(t.getClassList(n), r.__wbindgen_malloc),
                    i = H;
                  (k().setInt32(e + 4, i, !0), k().setInt32(e + 0, _, !0));
                }, arguments);
              },
              __wbg_get_507a50627bffa49b: function (e, t) {
                return e[t >>> 0];
              },
              __wbg_get_78f252d074a84d0b: function () {
                return F(function (e, t) {
                  return Reflect.get(e, t);
                }, arguments);
              },
              __wbg_has_8374cf06984d8bfc: function () {
                return F(function (e, t) {
                  return Reflect.has(e, t);
                }, arguments);
              },
              __wbg_host_18450e7fb2bf2108: function (e) {
                return e.host;
              },
              __wbg_insertRule_7f44b1a334b975c5: function () {
                return F(function (e, t, n, r) {
                  return e.insertRule(L(t, n), r >>> 0);
                }, arguments);
              },
              __wbg_item_6c9c57d2b0d3b03b: function (e, t) {
                let n = e.item(t >>> 0);
                return M(n) ? 0 : C(n);
              },
              __wbg_keys_58421f8f96795607: function (e) {
                return Object.keys(e);
              },
              __wbg_length_1f0964f4a5e2c6d8: function (e) {
                return e.length;
              },
              __wbg_length_370319915dc99107: function (e) {
                return e.length;
              },
              __wbg_length_c9d11fa3c1e97549: function (e) {
                return e.length;
              },
              __wbg_new_da52cf8fe3429cb2: function () {
                return {};
              },
              __wbg_new_from_slice_77cdfb7977362f3c: function (e, t) {
                return new Uint8Array(I(e, t));
              },
              __wbg_ownerDocument_5a7a5473f8709b3e: function (e) {
                let t = e.ownerDocument;
                return M(t) ? 0 : C(t);
              },
              __wbg_prototypesetcall_4770620bbe4688a0: function (e, t, n) {
                Uint8Array.prototype.set.call(I(e, t), n);
              },
              __wbg_publishEvent_749c038ad23b85ab: function (
                e,
                t,
                n,
                r,
                _,
                i,
                o,
                s,
                a,
                l,
              ) {
                e.publishEvent(
                  L(t, n),
                  0 === r ? void 0 : L(r, _),
                  i,
                  o >>> 0,
                  s,
                  a >>> 0,
                  l,
                );
              },
              __wbg_removeAttribute_1e7d2c409776d836: function () {
                return F(function (e, t, n) {
                  e.removeAttribute(L(t, n));
                }, arguments);
              },
              __wbg_removeAttribute_425b95bcfcd686d9: function () {
                return F(function (e, t, n, r) {
                  e.removeAttribute(t, L(n, r));
                }, arguments);
              },
              __wbg_removeProperty_70da952bc1b493fa: function () {
                return F(function (e, t, n, _) {
                  let i = z(
                      t.removeProperty(L(n, _)),
                      r.__wbindgen_malloc,
                      r.__wbindgen_realloc,
                    ),
                    o = H;
                  (k().setInt32(e + 4, o, !0), k().setInt32(e + 0, i, !0));
                }, arguments);
              },
              __wbg_runElementClosure_6b4e42c656f6ee9d: function (
                e,
                t,
                n,
                r,
                _,
                i,
                o,
              ) {
                e.runElementClosure(t, n, r >>> 0, _, i >>> 0, o);
              },
              __wbg_runWorklet_2a64b8ec4e735212: function (
                e,
                t,
                n,
                r,
                _,
                i,
                o,
              ) {
                e.runWorklet(t, n, r >>> 0, _, i >>> 0, o);
              },
              __wbg_setAttribute_39a501bc57b5d356: function () {
                return F(function (e, t, n, r, _, i) {
                  e.setAttribute(t, L(n, r), L(_, i));
                }, arguments);
              },
              __wbg_setAttribute_71039043be82d098: function () {
                return F(function (e, t, n, r, _) {
                  e.setAttribute(L(t, n), L(r, _));
                }, arguments);
              },
              __wbg_setProperty_e4e51b1b1d681d15: function () {
                return F(function (e, t, n, r, _) {
                  e.setProperty(L(t, n), L(r, _));
                }, arguments);
              },
              __wbg_set_8535240470bf2500: function () {
                return F(function (e, t, n) {
                  return Reflect.set(e, t, n);
                }, arguments);
              },
              __wbg_set_cssText_1d203a1b8ff80e20: function (e, t, n) {
                e.cssText = L(t, n);
              },
              __wbg_set_textContent_54dcad83ae15772d: function (e, t, n) {
                e.textContent = 0 === t ? void 0 : L(t, n);
              },
              __wbg_sheet_9201185a230c1cdc: function (e) {
                let t = e.sheet;
                return M(t) ? 0 : C(t);
              },
              __wbg_style_00ba0d9bec50983f: function (e) {
                return e.style;
              },
              __wbg_style_6657aed849e5d757: function (e) {
                return e.style;
              },
              __wbindgen_cast_0000000000000001: function (e, t) {
                return L(e, t);
              },
              __wbindgen_init_externref_table: function () {
                let e = r.__wbindgen_externrefs,
                  t = e.grow(4);
                (e.set(0, void 0),
                  e.set(t + 0, void 0),
                  e.set(t + 1, null),
                  e.set(t + 2, !0),
                  e.set(t + 3, !1));
              },
            },
          };
        }
        Symbol.dispose && (c.prototype[Symbol.dispose] = c.prototype.free);
        let m =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) => r.__wbg_eventinfo_free(e, 1)),
          v =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) =>
                  r.__wbg_mainthreadwasmcontext_free(e, 1),
                ),
          x =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) =>
                  r.__wbg_rawstyleinfo_free(e, 1),
                ),
          S =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) => r.__wbg_rule_free(e, 1)),
          E =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) => r.__wbg_ruleprelude_free(e, 1)),
          O =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) => r.__wbg_selector_free(e, 1)),
          R =
            'u' < typeof FinalizationRegistry
              ? { register: () => {}, unregister: () => {} }
              : new FinalizationRegistry((e) =>
                  r.__wbg_stylesheetresource_free(e, 1),
                );
        function C(e) {
          let t = r.__externref_table_alloc();
          return (r.__wbindgen_externrefs.set(t, e), t);
        }
        function U(e, t) {
          if (!(e instanceof t)) throw Error(`expected instance of ${t.name}`);
        }
        function A(e, t) {
          e >>>= 0;
          let n = k(),
            _ = [];
          for (let i = e; i < e + 4 * t; i += 4)
            _.push(r.__wbindgen_externrefs.get(n.getUint32(i, !0)));
          return (r.__externref_drop_slice(e, t), _);
        }
        function I(e, t) {
          return ((e >>>= 0), W().subarray(e / 1, e / 1 + t));
        }
        let j = null;
        function k() {
          return (
            (null === j ||
              !0 === j.buffer.detached ||
              (void 0 === j.buffer.detached && j.buffer !== r.memory.buffer)) &&
              (j = new DataView(r.memory.buffer)),
            j
          );
        }
        function L(e, t) {
          var n, r;
          return (
            (n = e >>> 0),
            (q += r = t) >= 0x7ff00000 &&
              ((B = new TextDecoder('utf-8', {
                ignoreBOM: !0,
                fatal: !0,
              })).decode(),
              (q = r)),
            B.decode(W().subarray(n, n + r))
          );
        }
        let T = null,
          N = null;
        function W() {
          return (
            (null === N || 0 === N.byteLength) &&
              (N = new Uint8Array(r.memory.buffer)),
            N
          );
        }
        function F(e, t) {
          try {
            return e.apply(this, t);
          } catch (t) {
            let e = C(t);
            r.__wbindgen_exn_store(e);
          }
        }
        function M(e) {
          return null == e;
        }
        function P(e, t) {
          let n = t(4 * e.length, 4) >>> 0;
          return (
            ((null === T || 0 === T.byteLength) &&
              (T = new Uint32Array(r.memory.buffer)),
            T).set(e, n / 4),
            (H = e.length),
            n
          );
        }
        function D(e, t) {
          let n = t(4 * e.length, 4) >>> 0;
          for (let t = 0; t < e.length; t++) {
            let r = C(e[t]);
            k().setUint32(n + 4 * t, r, !0);
          }
          return ((H = e.length), n);
        }
        function z(e, t, n) {
          if (void 0 === n) {
            let n = V.encode(e),
              r = t(n.length, 1) >>> 0;
            return (
              W()
                .subarray(r, r + n.length)
                .set(n),
              (H = n.length),
              r
            );
          }
          let r = e.length,
            _ = t(r, 1) >>> 0,
            i = W(),
            o = 0;
          for (; o < r; o++) {
            let t = e.charCodeAt(o);
            if (t > 127) break;
            i[_ + o] = t;
          }
          if (o !== r) {
            (0 !== o && (e = e.slice(o)),
              (_ = n(_, r, (r = o + 3 * e.length), 1) >>> 0));
            let t = W().subarray(_ + o, _ + r);
            ((o += V.encodeInto(e, t).written), (_ = n(_, r, o, 1) >>> 0));
          }
          return ((H = o), _);
        }
        function $(e) {
          let t = r.__wbindgen_externrefs.get(e);
          return (r.__externref_table_dealloc(e), t);
        }
        let B = new TextDecoder('utf-8', { ignoreBOM: !0, fatal: !0 });
        B.decode();
        let q = 0,
          V = new TextEncoder();
        'encodeInto' in V ||
          (V.encodeInto = function (e, t) {
            let n = V.encode(e);
            return (t.set(n), { read: e.length, written: n.length });
          });
        let H = 0;
        function G(e, t) {
          return (
            (r = e.exports),
            (j = null),
            (T = null),
            (N = null),
            r.__wbindgen_start(),
            r
          );
        }
        async function J(e, t) {
          if ('function' == typeof Response && e instanceof Response) {
            if ('function' == typeof WebAssembly.instantiateStreaming)
              try {
                return await WebAssembly.instantiateStreaming(e, t);
              } catch (t) {
                if (
                  e.ok &&
                  (function (e) {
                    switch (e) {
                      case 'basic':
                      case 'cors':
                      case 'default':
                        return !0;
                    }
                    return !1;
                  })(e.type) &&
                  'application/wasm' !== e.headers.get('Content-Type')
                )
                  console.warn(
                    '`WebAssembly.instantiateStreaming` failed because your server does not serve Wasm with `application/wasm` MIME type. Falling back to `WebAssembly.instantiate` which is slower. Original error:\n',
                    t,
                  );
                else throw t;
              }
            let n = await e.arrayBuffer();
            return await WebAssembly.instantiate(n, t);
          }
          {
            let n = await WebAssembly.instantiate(e, t);
            return n instanceof WebAssembly.Instance
              ? { instance: n, module: e }
              : n;
          }
        }
        function K(e) {
          if (void 0 !== r) return r;
          void 0 !== e &&
            (Object.getPrototypeOf(e) === Object.prototype
              ? ({ module: e } = e)
              : console.warn(
                  'using deprecated parameters for `initSync()`; pass a single object instead',
                ));
          let t = y();
          return (
            e instanceof WebAssembly.Module || (e = new WebAssembly.Module(e)),
            G(new WebAssembly.Instance(e, t), e)
          );
        }
        async function Q(e) {
          if (void 0 !== r) return r;
          (void 0 !== e &&
            (Object.getPrototypeOf(e) === Object.prototype
              ? ({ module_or_path: e } = e)
              : console.warn(
                  'using deprecated parameters for the initialization function; pass a single object instead',
                )),
            void 0 === e && (e = new URL(n(9099), n.b)));
          let t = y();
          ('string' == typeof e ||
            ('function' == typeof Request && e instanceof Request) ||
            ('function' == typeof URL && e instanceof URL)) &&
            (e = fetch(e));
          let { instance: _, module: i } = await J(await e, t);
          return G(_, i);
        }
        n.d(t, {
          EventInfo: () => _,
          MainThreadWasmContext: () => i,
          RawStyleInfo: () => o,
          Rule: () => s,
          RulePrelude: () => a,
          Selector: () => l,
          StyleSheetResource: () => c,
          add_inline_style_raw_string_key: () => u,
          decode_style_info: () => f,
          default: () => Q,
          encode_legacy_json_generated_raw_style_info: () => b,
          get_font_face_content: () => d,
          get_style_content: () => g,
          initSync: () => K,
          set_inline_styles_in_key_value_vec: () => w,
          set_inline_styles_in_str: () => p,
          set_inline_styles_number_key: () => h,
        });
      },
      5690(e, t, n) {
        n.a(e, async function (e, r) {
          try {
            var _ = n(4785),
              i = e([_]);
            function o(e, t, n, r, i, o) {
              let s = new _.wasmInstance.RawStyleInfo();
              for (let [t, n] of Object.entries(e)) {
                let e = parseInt(t, 10);
                if (
                  (n.imports &&
                    n.imports.forEach((t) => {
                      let n = parseInt(t, 10);
                      isNaN(n) || s.append_import(e, n);
                    }),
                  n.content)
                ) {
                  let t = n.content.join('\n').trim();
                  t.length > 0 &&
                    (function (e, t, n) {
                      let r = new _.wasmInstance.Rule('StyleRule'),
                        i = new _.wasmInstance.RulePrelude(),
                        o = new _.wasmInstance.Selector();
                      (o.push_one_selector_section('UnknownText', '{}' + n),
                        i.push_selector(o),
                        r.set_prelude(i),
                        e.push_rule(t, r));
                    })(s, e, t);
                }
                for (let t of n.rules) {
                  let n = new _.wasmInstance.Rule('StyleRule');
                  for (let [e, r] of t.decl) n.push_declaration(e, r);
                  let r = new _.wasmInstance.RulePrelude();
                  for (let e of t.sel) {
                    let t = new _.wasmInstance.Selector();
                    for (let n = 0; n < e.length; n += 4) {
                      let r = e[n] || [],
                        _ = e[n + 1] || [],
                        i = e[n + 2] || [],
                        o = e[n + 3] || [];
                      for (let e of r)
                        !(function (e, t) {
                          if (t.startsWith('.'))
                            e.push_one_selector_section(
                              'ClassSelector',
                              t.substring(1),
                            );
                          else if (t.startsWith('#'))
                            e.push_one_selector_section(
                              'IdSelector',
                              t.substring(1),
                            );
                          else if (
                            t.startsWith('[') &&
                            t.startsWith('[lynx-tag=') &&
                            t.endsWith(']')
                          ) {
                            let n = t.substring(10, t.length - 1);
                            if (
                              (((n.startsWith('"') && n.endsWith('"')) ||
                                (n.startsWith("'") && n.endsWith("'"))) &&
                                (n = n.substring(1, n.length - 1)),
                              'page' === n)
                            )
                              e.push_one_selector_section(
                                'AttributeSelector',
                                'part="page"',
                              );
                            else {
                              let t = n.includes('-') ? n : `x-${n}`;
                              e.push_one_selector_section('TypeSelector', t);
                            }
                          } else if (t.startsWith('[')) {
                            let n = t.substring(1, t.length - 1);
                            e.push_one_selector_section('AttributeSelector', n);
                          } else
                            '*' === t
                              ? e.push_one_selector_section(
                                  'UniversalSelector',
                                  '*',
                                )
                              : e.push_one_selector_section('TypeSelector', t);
                        })(t, e);
                      for (let e of _)
                        if ('::part(input)::placeholder' === e)
                          t.push_one_selector_section(
                            'PseudoElementSelector',
                            'placeholder',
                          );
                        else {
                          let n = e.startsWith(':') ? e.substring(1) : e;
                          t.push_one_selector_section('PseudoClassSelector', n);
                        }
                      for (let e of i) {
                        let n = e.startsWith('::')
                          ? e.substring(2)
                          : e.startsWith(':')
                            ? e.substring(1)
                            : e;
                        t.push_one_selector_section('PseudoElementSelector', n);
                      }
                      o.length > 0 &&
                        t.push_one_selector_section('Combinator', o[0]);
                    }
                    r.push_selector(t);
                  }
                  (n.set_prelude(r), s.push_rule(e, n));
                }
              }
              return _.wasmInstance.encode_legacy_json_generated_raw_style_info(
                s,
                t,
                o,
                n,
                r,
                i,
              );
            }
            ((_ = (i.then ? (await i)() : i)[0]), n.d(t, { g: () => o }), r());
          } catch (e) {
            r(e);
          }
        });
      },
      7504(e, t, n) {
        n.a(e, async function (e, t) {
          try {
            let f, b;
            var r = n(1980),
              _ = n(4785),
              i = n(5690),
              o = n(9932),
              s = e([_, i]);
            [_, i] = s.then ? (await s)() : s;
            let d = new Promise((e) => {
                f = e;
              }),
              g =
                '//# allFunctionsCalledOnLoad\n(function(){ "use strict"; const navigator=void 0,postMessage=void 0; let window=void 0; ';
            class w {
              #e;
              #t = new Uint8Array(0);
              constructor(e) {
                this.#e = e;
              }
              async read(e) {
                if (this.#t.length >= e) {
                  let t = this.#t.slice(0, e);
                  return ((this.#t = this.#t.slice(e)), t);
                }
                for (; this.#t.length < e;) {
                  let { done: e, value: t } = await this.#e.read();
                  if (t) {
                    let e = new Uint8Array(this.#t.length + t.length);
                    (e.set(this.#t), e.set(t, this.#t.length), (this.#t = e));
                  }
                  if (e) break;
                }
                if (this.#t.length < e) {
                  if (0 === this.#t.length) return null;
                  throw Error(
                    `Unexpected end of stream. Expected ${e} bytes, got ${this.#t.length}`,
                  );
                }
                let t = this.#t.slice(0, e);
                return ((this.#t = this.#t.slice(e)), t);
              }
              async readRest() {
                for (;;) {
                  let { done: e, value: t } = await this.#e.read();
                  if (t) {
                    let e = new Uint8Array(this.#t.length + t.length);
                    (e.set(this.#t), e.set(t, this.#t.length), (this.#t = e));
                  }
                  if (e) break;
                }
                let e = this.#t;
                return ((this.#t = new Uint8Array(0)), e);
              }
            }
            function a(e) {
              let t = new Uint16Array(e.buffer, e.byteOffset, e.byteLength / 2),
                n = '';
              for (let e = 0; e < t.length; e += 8192)
                n += String.fromCharCode.apply(null, t.subarray(e, e + 8192));
              return JSON.parse(n);
            }
            function l() {
              if (void 0 === b) {
                var e;
                ((e = b =
                  setTimeout(() => {
                    ((b = void 0), postMessage({ type: 'heartbreak' }));
                  }, 1e3)),
                  'object' == typeof e &&
                    null !== e &&
                    'unref' in e &&
                    e.unref());
              }
            }
            async function c(e, t, n, i, s, l) {
              let c = new w(t),
                f = {},
                b = await c.read(8);
              if (!b) throw Error('Empty stream');
              if (123 === b[0]) {
                let t = await c.readRest(),
                  r = new TextDecoder(),
                  _ = r.decode(b) + r.decode(t),
                  o = JSON.parse(_);
                await u(o, e, n, i, s, l);
                return;
              }
              let p = new DataView(b.buffer, b.byteOffset, b.byteLength),
                h = p.getUint32(0, !0),
                y = p.getUint32(4, !0);
              if (h !== r.Bz || y !== r.ug) throw Error('Invalid Magic Header');
              let m = await c.read(4);
              if (!m) throw Error('Unexpected EOF reading version');
              let v = new DataView(
                m.buffer,
                m.byteOffset,
                m.byteLength,
              ).getUint32(0, !0);
              if (v > 1) throw Error(`Unsupported version: ${v}`);
              for (;;) {
                let t = await c.read(4);
                if (!t) break;
                let u = new DataView(
                    t.buffer,
                    t.byteOffset,
                    t.byteLength,
                  ).getUint32(0, !0),
                  b = await c.read(4);
                if (!b) throw Error('Unexpected EOF reading section length');
                let w = new DataView(
                    b.buffer,
                    b.byteOffset,
                    b.byteLength,
                  ).getUint32(0, !0),
                  p = await c.read(w);
                if (!p)
                  throw Error(
                    `Unexpected EOF reading section content. Expected ${w} bytes.`,
                  );
                switch (u) {
                  case r.eC.Configurations:
                    ((f = l ? { ...a(p), ...l } : a(p)),
                      postMessage({
                        type: 'section',
                        label: u,
                        url: e,
                        data: f,
                      }));
                    break;
                  case r.eC.StyleInfo: {
                    await d;
                    let t = _.wasmInstance.decode_style_info(
                      p,
                      'true' === f.isLazy ? e : void 0,
                      'true' === f.enableCSSSelector,
                      n,
                      i,
                      s,
                    );
                    postMessage(
                      {
                        type: 'section',
                        label: u,
                        url: e,
                        data: t.buffer,
                        config: f,
                      },
                      { transfer: [t.buffer] },
                    );
                    break;
                  }
                  case r.eC.LepusCode: {
                    let t = (0, o.E)(p),
                      n = 'true' === f.isLazy,
                      r =
                        'true' === f.isExternalBundle
                          ? 'var exports=(module.exports={}); '
                          : n
                            ? 'module.exports='
                            : '',
                      _ = {};
                    for (let [n, i] of Object.entries(t)) {
                      let t = new Blob(
                        [g, r, i, ' \n })()\n//# sourceURL=', e, '/', n, '\n'],
                        { type: 'text/javascript; charset=utf-8' },
                      );
                      _[n] = URL.createObjectURL(t);
                    }
                    postMessage({
                      type: 'section',
                      label: u,
                      url: e,
                      data: _,
                      config: f,
                    });
                    break;
                  }
                  case r.eC.ElementTemplates:
                    postMessage(
                      { type: 'section', label: u, url: e, data: p },
                      [p.buffer],
                    );
                    break;
                  case r.eC.CustomSections:
                    postMessage(
                      { type: 'section', label: u, url: e, data: p.buffer },
                      { transfer: [p.buffer] },
                    );
                    break;
                  case r.eC.Manifest: {
                    let t = (0, o.E)(p),
                      n = {};
                    for (let [r, _] of Object.entries(t)) {
                      let t = new Blob([_, '//# sourceURL=', e, '/', r], {
                        type: 'text/javascript; charset=utf-8',
                      });
                      n[r] = URL.createObjectURL(t);
                    }
                    postMessage({ type: 'section', label: u, url: e, data: n });
                    break;
                  }
                  default:
                    throw Error(`Unknown section label: ${u}`);
                }
              }
            }
            async function u(e, t, n, _, o, s) {
              let a = {};
              if (
                (e.pageConfig && (a = { ...e.pageConfig }),
                e.lepusCode?.root && 'string' == typeof e.lepusCode.root)
              ) {
                let t =
                  e.appType ??
                  (e.lepusCode.root.startsWith(
                    '(function (globDynamicComponentEntry',
                  )
                    ? 'lazy'
                    : 'card');
                ((a.cardType = e.cardType ?? e.pageConfig?.cardType ?? 'react'),
                  (a.appType = a.appType ?? t),
                  (a.isLazy = 'card' === t ? 'false' : 'true'));
              }
              if (
                (s && (a = { ...a, ...s }),
                (a = Object.fromEntries(
                  Object.entries(a).map(([e, t]) => [e, t.toString()]),
                )),
                postMessage({
                  type: 'section',
                  label: r.eC.Configurations,
                  url: t,
                  data: a,
                }),
                e.styleInfo)
              ) {
                await d;
                let s = (0, i.g)(
                  e.styleInfo,
                  'true' === a.enableCSSSelector,
                  n,
                  _,
                  o,
                  'true' === a.isLazy ? t : void 0,
                );
                postMessage(
                  {
                    type: 'section',
                    label: r.eC.StyleInfo,
                    url: t,
                    data: s.buffer,
                    config: a,
                  },
                  { transfer: [s.buffer] },
                );
              }
              if (e.lepusCode) {
                let n = 'true' === a.isLazy,
                  _ = {};
                for (let [r, i] of Object.entries(e.lepusCode)) {
                  if ('string' != typeof i) continue;
                  let e = `${g}${n ? 'module.exports=' : ''} `,
                    o = ` 
 })()
//# sourceURL=${t}/${r}
`,
                    s = new Blob([e, i, o], {
                      type: 'text/javascript; charset=utf-8',
                    });
                  _[r] = URL.createObjectURL(s);
                }
                postMessage({
                  type: 'section',
                  label: r.eC.LepusCode,
                  url: t,
                  data: _,
                  config: a,
                });
              }
              if (e.manifest) {
                let n = {};
                for (let [t, r] of Object.entries(e.manifest)) {
                  if ('string' != typeof r) continue;
                  let e = new Blob([r], { type: 'text/javascript;' });
                  n[t] = URL.createObjectURL(e);
                }
                postMessage({
                  type: 'section',
                  label: r.eC.Manifest,
                  url: t,
                  data: n,
                });
              }
              if (
                (e.customSections &&
                  postMessage({
                    type: 'section',
                    label: r.eC.CustomSections,
                    url: t,
                    data: e.customSections,
                  }),
                e.elementTemplates &&
                  Object.keys(e.elementTemplates).length > 0)
              )
                throw Error(
                  'ElementTemplates in JSON artifacts are not supported yet.',
                );
            }
            ((self.onmessage = async (e) => {
              let t = e.data;
              if ('init' === t.type) {
                let { wasmModule: e } = t;
                (_.wasmInstance.initSync({ module: e }), f());
              } else if ('heartbreak' === t.type) l();
              else if ('load' === t.type) {
                let {
                  url: e,
                  fetchUrl: n,
                  overrideConfig: r,
                  transformVW: _,
                  transformVH: i,
                  transformREM: o,
                } = t;
                try {
                  let t = await fetch(n, {
                    headers: {
                      Accept: 'application/octet-stream, application/json',
                    },
                  });
                  if (!t.body || 200 !== t.status)
                    throw Error(`Failed to fetch template: ${t.statusText}`);
                  let s = t.body.getReader();
                  (await c(e, s, _, i, o, r),
                    postMessage({ type: 'done', url: e }));
                } catch (t) {
                  postMessage({ type: 'error', url: e, error: t.message });
                }
              }
            }),
              postMessage({ type: 'ready' }),
              l(),
              t());
          } catch (e) {
            t(e);
          }
        });
      },
      4785(e, t, n) {
        n.a(
          e,
          async function (e, r) {
            try {
              var _ = n(480);
              let e =
                  'u' > typeof WorkerGlobalScope &&
                  self instanceof WorkerGlobalScope,
                i = Promise.all([(0, _.rO)(), (0, _.oR)()]).then(([t, r]) =>
                  t && r
                    ? Promise.all([
                        Promise.resolve().then(n.bind(n, 8235)),
                        e
                          ? void 0
                          : WebAssembly.compileStreaming(
                              fetch(new URL(n(9099), n.b)),
                            ),
                      ])
                    : Promise.all([
                        n.e(778, 'low').then(n.bind(n, 9125)),
                        e
                          ? void 0
                          : WebAssembly.compileStreaming(
                              fetch(new URL(n(8331), n.b)),
                            ),
                      ]),
                ),
                [o, s] = await i;
              (e || (await o.default(s)), n.d(t, {}, { wasmInstance: o }), r());
            } catch (e) {
              r(e);
            }
          },
          1,
        );
      },
      9932(e, t, n) {
        function r(e) {
          let t = new DataView(e.buffer, e.byteOffset, e.byteLength),
            n = 0;
          if (e.byteLength < 4) throw Error('Buffer too short for count');
          let r = t.getUint32(n, !0);
          n += 4;
          let _ = {},
            i = new TextDecoder();
          for (let o = 0; o < r; o++) {
            if (e.byteLength < n + 4)
              throw Error('Buffer too short for key length');
            let r = t.getUint32(n, !0);
            if (((n += 4), e.byteLength < n + r))
              throw Error('Buffer too short for key');
            let o = i.decode(e.subarray(n, n + r));
            if (((n += r), e.byteLength < n + 4))
              throw Error('Buffer too short for value length');
            let s = t.getUint32(n, !0);
            if (((n += 4), e.byteLength < n + s))
              throw Error('Buffer too short for value');
            let a = e.subarray(n, n + s);
            ((n += s), (_[o] = a));
          }
          return _;
        }
        n.d(t, { E: () => r });
      },
      1980(e, t, n) {
        var r, _, i, o, s, a;
        (Object.entries({
          click: 'tap',
          lynxscroll: 'scroll',
          lynxscrollend: 'scrollend',
          overlaytouch: 'touch',
          lynxfocus: 'focus',
          lynxblur: 'blur',
          lynxinput: 'input',
        }).map(([e, t]) => [t, e]),
          ((o = r || (r = {}))[(o.SUCCESS = 0)] = 'SUCCESS'),
          (o[(o.UNKNOWN = 1)] = 'UNKNOWN'),
          (o[(o.NODE_NOT_FOUND = 2)] = 'NODE_NOT_FOUND'),
          (o[(o.METHOD_NOT_FOUND = 3)] = 'METHOD_NOT_FOUND'),
          (o[(o.PARAM_INVALID = 4)] = 'PARAM_INVALID'),
          (o[(o.SELECTOR_NOT_SUPPORTED = 5)] = 'SELECTOR_NOT_SUPPORTED'),
          (o[(o.NO_UI_FOR_NODE = 6)] = 'NO_UI_FOR_NODE'));
        let l = Object.freeze(
          Object.assign(Object.create(null), {
            view: 'x-view',
            text: 'x-text',
            image: 'x-image',
            'raw-text': 'raw-text',
            'scroll-view': 'x-scroll-view',
            wrapper: 'lynx-wrapper',
            list: 'x-list',
            page: 'div',
            input: 'x-input',
            'x-input-ng': 'x-input',
            textarea: 'x-textarea',
            svg: 'x-svg',
            frame: 'lynx-view',
          }),
        );
        (Object.assign(
          Object.create(null),
          Object.fromEntries(Object.entries(l).map(([e, t]) => [t, e])),
        ),
          Object.assign(Object.create(null), {
            list: 0,
            'x-swiper': 1,
            'x-input': 2,
            'x-input-ng': 2,
            input: 2,
            'x-textarea': 3,
            'x-audio-tt': 4,
            'x-foldview-ng': 5,
            'x-foldview-header-ng': 5,
            'x-foldview-slot-drag-ng': 5,
            'x-foldview-slot-ng': 5,
            'x-foldview-toolbar-ng': 5,
            'x-refresh-view': 6,
            'x-refresh-header': 6,
            'x-refresh-footer': 6,
            'x-overlay-ng': 7,
            'x-viewpager-ng': 8,
            'x-viewpager-item-ng': 8,
          }),
          Symbol.for('lynx-scroll-container-dom'),
          ((s = _ || (_ = {}))[(s.ID_SELECTOR = 0)] = 'ID_SELECTOR'),
          (s[(s.REF_ID = 1)] = 'REF_ID'),
          (s[(s.UNIQUE_ID = 2)] = 'UNIQUE_ID'),
          ((a = i || (i = {}))[(a.START = 0)] = 'START'),
          (a[(a.PLAY = 1)] = 'PLAY'),
          (a[(a.PAUSE = 2)] = 'PAUSE'),
          (a[(a.CANCEL = 3)] = 'CANCEL'),
          (a[(a.FINISH = 4)] = 'FINISH'),
          n.d(
            t,
            {},
            {
              Bz: 0x41524453,
              eC: {
                Manifest: 1,
                StyleInfo: 2,
                LepusCode: 3,
                CustomSections: 4,
                ElementTemplates: 5,
                Configurations: 6,
              },
              ug: 0x464f5257,
            },
          ));
      },
      480(e, t, n) {
        let r = async () =>
            WebAssembly.validate(
              new Uint8Array([
                0, 97, 115, 109, 1, 0, 0, 0, 1, 4, 1, 96, 0, 0, 3, 2, 1, 0, 10,
                7, 1, 5, 0, 208, 112, 26, 11,
              ]),
            ),
          _ = async () =>
            WebAssembly.validate(
              new Uint8Array([
                0, 97, 115, 109, 1, 0, 0, 0, 1, 5, 1, 96, 0, 1, 123, 3, 2, 1, 0,
                10, 10, 1, 8, 0, 65, 0, 253, 15, 253, 98, 11,
              ]),
            );
        n.d(t, {}, { oR: _, rO: r });
      },
    },
    u = {};
  function f(e) {
    var t = u[e];
    if (void 0 !== t) return t.exports;
    var n = (u[e] = { exports: {} });
    return (c[e](n, n.exports, f), n.exports);
  }
  ((f.m = c),
    (f.c = u),
    (t = (e = 'function' == typeof Symbol)
      ? Symbol('rspack queues')
      : '__rspack_queues'),
    (n = f.aE = e ? Symbol('rspack exports') : '__webpack_exports__'),
    (r = e ? Symbol('rspack error') : '__rspack_error'),
    (_ = e ? Symbol('rspack done') : '__rspack_done'),
    (i = f.zS = e ? Symbol('rspack defer') : '__rspack_defer'),
    (f.zT = (e) => {
      if (
        e.some((e) => {
          var t = u[e];
          return !t || !1 === t[_];
        })
      )
        return { then: (t, n) => Promise.all(e.map(f)).then(t, n) };
    }),
    (o = (e) => {
      e &&
        e.d < 1 &&
        ((e.d = 1),
        e.forEach((e) => e.r--),
        e.forEach((e) => (e.r-- ? e.r++ : e())));
    }),
    (f.a = (e, s, a) => {
      a && ((l = []).d = -1);
      var l,
        c,
        u,
        b,
        d = new Set(),
        g = e.exports,
        w = new Promise((e, t) => {
          ((b = t), (u = e));
        });
      ((w[n] = g),
        (w[t] = (e) => {
          (l && e(l), d.forEach(e), w.catch(() => {}));
        }),
        (e.exports = w),
        s(
          (e) => {
            c = e.map((e) => {
              if (null !== e && 'object' == typeof e) {
                if (!e[t] && e[i]) {
                  var _ = f.zT(e[i]);
                  if (!_) return e;
                  var s = e;
                  e = {
                    then(e, t) {
                      _.then(() => e(s), t);
                    },
                  };
                }
                if (e[t]) return e;
                if (e.then) {
                  var a = [];
                  ((a.d = 0),
                    e.then(
                      (e) => {
                        ((l[n] = e), o(a));
                      },
                      (e) => {
                        ((l[r] = e), o(a));
                      },
                    ));
                  var l = {};
                  return ((l[i] = !1), (l[t] = (e) => e(a)), l);
                }
              }
              var c = {};
              return ((c[t] = () => {}), (c[n] = e), c);
            });
            var _,
              s = () =>
                c.map((e) => {
                  if (e[i]) return e;
                  if (e[r]) throw e[r];
                  return e[n];
                }),
              a = new Promise((e) => {
                (_ = () => e(s)).r = 0;
                var n = (e) =>
                  e !== l &&
                  !d.has(e) &&
                  (d.add(e), e && !e.d && (_.r++, e.push(_)));
                c.map((e) => e[i] || e[t](n));
              });
            return _.r ? a : s();
          },
          (e) => (e ? b((w[r] = e)) : u(g), o(l), (w[_] = !0)),
        ),
        l && l.d < 0 && (l.d = 0));
    }),
    (f.d = (e, t, n) => {
      var r = (t, n) => {
        for (var r in t)
          f.o(t, r) &&
            !f.o(e, r) &&
            Object.defineProperty(e, r, { enumerable: !0, [n]: t[r] });
      };
      (r(t, 'get'), r(n, 'value'));
    }),
    (f.f = {}),
    (f.e = (e, t) =>
      Promise.all(Object.keys(f.f).reduce((n, r) => (f.f[r](e, n, t), n), []))),
    (f.u = (e) => 'static/js/async/legacy-wasm-js.js'),
    (f.miniCssF = (e) => '' + e + '.css'),
    (f.o = (e, t) => Object.prototype.hasOwnProperty.call(e, t)),
    (f.r = (e) => {
      ('u' > typeof Symbol &&
        Symbol.toStringTag &&
        Object.defineProperty(e, Symbol.toStringTag, { value: 'Module' }),
        Object.defineProperty(e, '__esModule', { value: !0 }));
    }),
    (f.p = '/'),
    (f.b = self.location + '/../../../../'),
    (s = { 350: 1 }),
    (l = (a = globalThis.rspackChunk_lynx_js_web_rsbuild_server_middleware ||=
      []).push.bind(a)),
    (a.push = (e) => {
      var [t, n, r] = e;
      for (var _ in n) f.o(n, _) && (f.m[_] = n[_]);
      for (r && r(f); t.length;) s[t.pop()] = 1;
      l(e);
    }),
    (f.f.i = (e, t) => {
      s[e] || importScripts(f.p + f.u(e));
    }),
    f(7504));
})();
