    ...
    <div className="max-w-3xl mx-auto space-y-8 animate-in fade-in pb-12">
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-tr from-indigo-600 to-violet-600 shadow-md flex items-center justify-center text-2xl font-black text-white">
            {user?.name?.charAt(0).toUpperCase() || 'U'}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-indigo-700 uppercase tracking-widest bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">
            ...
            <h1 className="text-2xl font-extrabold text-slate-900 mt-1.5">{user?.name}</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-100 text-sm">
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <p className="font-bold text-slate-900 mt-1 font-mono">{user?.nip_nik_nisn || user?.nisn || user?.nip || '-'}</p>
          </div>

          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
            <span className="text-xs text-slate-500 uppercase font-bold">Rombongan Belajar (Rombel)</span>
            <p className="font-bold text-slate-900 mt-1">{user?.rombel || 'Semua Rombel'}</p>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
            ...
            <p className="font-bold text-slate-900 mt-1">
              {user?.is_initial_password ? (
                <span className="text-amber-600">Password Bawaan (Disarankan Ubah)</span>
              ) : (
                <span className="text-emerald-600 font-semibold">Telah Diperbarui</span>
              )}
            </p>
          </div>
        </div>
      ...
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
        ...
        <form onSubmit={handleChangePassword} className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 uppercase">Kata Sandi Saat Ini</label>
            <input
              type="password"
              value={currentPass}
              ...
              required
              placeholder="••••••••"
              className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-indigo-600 rounded-xl p-3 text-sm text-slate-900 focus:outline-none shadow-xs"
            />
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 uppercase">Kata Sandi Baru</label>
              <input
                type="password"
                value={newPass}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-slate-700 uppercase">Konfirmasi Kata Sandi Baru</label>
              <input
                type="password"
                value={confirmPass}
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3.5 bg-[#1e1b4b] hover:bg-slate-900 text-white font-bold rounded-xl shadow-md shadow-indigo-100 transition-colors cursor-pointer disabled:opacity-50"
          >
